...
- **PII minimization** — configurable detectors for email, phone, government-ID-shaped values (US SSN), API keys/bearer tokens, and banking identifiers. Detection emits type/location/severity/confidence; redaction runs before audit hash/persist and before any telemetry export (previews + stable hashes only). Default action is **redact** (does not change allow/block); processes may opt into `block` or `escalate`. See [docs/sensitive-data.md](docs/sensitive-data.md).
